...
我们知道类的静态函数是没有this指针的，调用它时不需要创建对象，通过：类名 ：：函数名（参数）的形式直接调用。静态函数只有**唯一的**一份，因此它的**地址是固定不变的**， 所以编译的时候但凡遇到调用该静态函数的时候就知道调用的是哪一个函数，因此说**静态函数在编译的时候就已经确定运行时机。** 而虚函数则不然，看下面的代码：
...
![img](https://img-blog.csdn.net/20150614001201106?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvc3VubnkwNA==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/Center)





![image-20200805093916697](E:\0OneDrive\OneDrive\Work\CSNote\0other\0\pics\image-20200805093916697.png)
